...
密钥不只在本地加密，还能**上链存证**——emoji 表情可视化防篡改 + IPFS 去中心化备份 + OpenTimestamps 比特币时间戳。

### 表情比对防篡改

```
本地密钥数据 → SHA-256 哈希 → 映射成 8 个 emoji 表情
同时上传服务器 + IPFS 生成比对页面
...
```bash
keynl chain        # 上链校验：上传哈希生成比对页（菜单16）
keynl query        # 泄露查询：显示本地表情+链上链接（菜单17）
keynl chain-file   # 文件上链：任意文件算哈希上链（菜单20）
keynl mychain      # 我的链上密钥：查看/抹除链上记录（菜单19）
keynl ots-verify   # 验证OTS：验证比特币时间戳（菜单21）
```
...
| 字段 | 说明 |
|:---|:---|
| 原始哈希 | 数据完整 SHA-256（64字符） |
| emoji 表情 | 8个表情，肉眼比对 |
| 生成时间 | 时间摘要 |
| 前块哈希 | 区块链接，防整段替换 |
| ECC 签名 | 服务器私钥签名，防伪造 |
| IPFS 地址 | 去中心化备份（服务器挂了锚点仍在） |
| OTS 存证 | OpenTimestamps 比特币时间戳（证明"X时刻已存在"） |

### 存证架构

```
中心化:   furrynaling.com/chain/<哈希>.html（表情比对页）
去中心化: IPFS（锚点 JSON，内容哈希即地址）
比特币:   OpenTimestamps（.ots 证明，提交公共日历）
```
